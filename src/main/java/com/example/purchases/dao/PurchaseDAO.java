...
public class PurchaseDAO {
    ...
    public Integer addPurchase(Purchase purchase) {
        jdbcTemplate.update(String.format("INSERT purchase(name, user_login) VALUES ('%s', '%s');", purchase.getName(), purchase.getUserLogin()));
    ...
    public List<Purchase> getPurchasesByUserLogin(String login) {
        return jdbcTemplate.query("SELECT * FROM purchase WHERE user_login = '" + login + "';", (result, rowNum) ->
    ...
    public void setPurchaseMark(int id, boolean isMarked) {
        jdbcTemplate.update(String.format("UPDATE purchase SET marked = %d WHERE id = %d;", isMarked ? 1 : 0, id));
    }

    public void deletePurchase(int id) {
        jdbcTemplate.update(String.format("DELETE FROM purchase WHERE id = %d", id));
    }
